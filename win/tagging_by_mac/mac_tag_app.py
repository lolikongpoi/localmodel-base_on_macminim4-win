...
class MacTagApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.start_button = ttk.Button(actions, text="开始批量打 Tag", command=self._start_tagging)
        self.start_button.pack(side="right")
        self.pause_button = ttk.Button(actions, text="暂停（当前图片完成后）", command=self._toggle_pause, state="disabled")
        self.pause_button.pack(side="right", padx=(0, 8))
        self.cancel_button = ttk.Button(actions, text="停止（当前图片完成后）", command=self._cancel, state="disabled")
