...
6. 需要整理旧报告时，点击“清理已归档报告”。程序会先核验归档状态，再要求确认。
...
“清理已归档报告”仅会删除完整且已经安全归档的报告运行目录。它要求报告已生成 `run_summary.json`、CSV 中全部为 `success`，且每张图片均在数据库登记为 `moved`。含失败记录、仍在运行、格式异常或尚有图片未归档的报告会保留。

清理前会显示报告数量与记录数并再次确认。清理只删除报告中的 CSV、JSONL 与摘要，不会删除目标图片、F 盘缓存数据库或 `G:\image` 中的 SQLite / CSV 标签库。
